...
| image.repository | string | `"ghcr.io/ritense/operaton-cockpit"` |  |
...
| settings.plugin.useEmailAsOperatonUserId | bool | `false` |  |
| settings.plugin.useGroupPathAsOperatonGroupId | bool | `true` |  |
| settings.plugin.useUsernameAsOperatonUserId | bool | `true` |  |
| settings.server.port | int | `8084` |  |
| settings.server.servlet.contextPath | string | `"/"` |  |
| settings.spring.application.name | string | `"operaton-cockpit"` |  |
...
| settings.spring.keycloak.clientId | string | `"operaton-identity-service"` |  |
